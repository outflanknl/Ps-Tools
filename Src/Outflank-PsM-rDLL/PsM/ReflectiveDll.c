...
BOOL EnumPeb(HANDLE hProcess) {
	...
	PEB_LDR_DATA2 pld;
	LDR_DATA_TABLE_ENTRY2 ldte;
	WCHAR lpwFullDllName[MAX_PATH] = { 0 };

	status = NtReadVirtualMemory(hProcess, &peb.Ldr->InLoadOrderModuleList, &peb.Ldr->InLoadOrderModuleList, sizeof(LPVOID), NULL);
	if (status != STATUS_SUCCESS) {
		return FALSE;
	}

	status = NtReadVirtualMemory(hProcess, peb.Ldr, &pld, sizeof(pld), NULL);
	if (status != STATUS_SUCCESS) {
		return FALSE;
	}
	...
	do
	{
		status = NtReadVirtualMemory(hProcess, pNextModuleInfo, &ldte, sizeof(ldte), NULL);
		if (status != STATUS_SUCCESS) {
			return FALSE;
		}

		status = NtReadVirtualMemory(hProcess, (LPVOID)ldte.FullDllName.Buffer, (LPVOID)lpwFullDllName, ldte.FullDllName.Length, NULL);
		if (status != STATUS_SUCCESS) {
			return FALSE;
		}
		...
		RtlZeroMemory(lpwFullDllName, MAX_PATH);
